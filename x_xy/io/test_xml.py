...
def test_xml_parsing():
    for example_name in x_xy.io.list_examples():
        ...
        print(original_sys.qd_size(), original_sys.link_armature,
              compare_sys.qd_size(), compare_sys.link_armature)
    ...
    def double_load_xml_to_sys(xml_path: Path) -> System:
        ...
        exported_xml = x_xy.io.xml.system_to_xml_str(orig_sys)
        new_sys = x_xy.io.xml.load_sys_from_str(exported_xml)
        return new_sys

    sys_test_xml_0 = double_load_xml_to_sys("examples/test_all_0.xml")
    ...
    assert not System.deep_equal(sys_test_xml_0, sys_test_xml_1)
    assert not System.deep_equal(sys_test_xml_0, sys_test_xml_2)
